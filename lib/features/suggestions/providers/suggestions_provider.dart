...
final suggestionsProvider =
    FutureProvider.family<List<Manga>, String?>((ref, genre) async {
  final source = ref.watch(currentSourceProvider);

  if (genre != null) {
    final results = await source.searchMangaByTags([genre]);
  ...
  if (topTags.isNotEmpty) {
    final results = await source.searchMangaByTags(topTags);
    if (results.isNotEmpty) return results;
  }

  // Fallback: popular manga.
  return source.getPopularManga(page: 1);
});
...
final genreTagsProvider = FutureProvider<List<String>>((ref) async {
  final source = ref.watch(currentSourceProvider);
  return source.getAvailableTags();
});
